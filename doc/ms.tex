...
In Figure~\ref{fig:piro} we compare model calculations from
...
Figure~\ref{fig:piro}. In fact, if we adopt $t_\mathrm{exp} = t_0 - 0.4 \,
\mathrm{d}$, we find that the model with the strongest mixing (orange line in
Figure~\ref{fig:piro}) provides an excellent match to the $g$-band
observations.
...
in the UV. The models shown in Figure~\ref{fig:piro} also overpredict the
