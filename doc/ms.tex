...
\begin{figure}[!thb]
  ...
  with the early $g_\mathrm{PTF}$ light curve of \abc. 
  Detections are shown as circles, while 3$\sigma$
  upper limits are shown as downward facing arrows.
  expected $g_\mathrm{PTF}$ light curve for the smallest orbital 
  separation capable of explaining the initial detection of \abc. 
  This model also requires $t_\mathrm{exp} \approx t_0 - 1 \; 
  \mathrm{d}$. Assuming $t_\mathrm{exp} = t_0$, interaction with a 
  companion at $a = 2 \times 10^{13} \; \mathrm{cm}$ matches the 
  initial detection of \abc\ (solid line). This model 
  significantly overestimates the UV flux detected from \abc\ 
  $\sim$3 d after explosion (see text). The $g_\mathrm{PTF}$ 
  observations $\sim$1--3~d after $t_0$ are well matched to 
  companion-interaction models for $a = 10^{14} \; \mathrm{cm}$ 
  if $t_\mathrm{exp} \approx t_0 + 0.3 \; \mathrm{d}$ (dash-dot 
  line). This model significantly underestimates the flux at the 
  epoch of discovery, while also overestimating the UV flux. Thus, 
  none of these possibilities uniquely explain the early flux from 
  \abc.
  \amiller{Add Swift light curve and photometry to make it clear that two large companions don't explain the observations? Also -- make labels for this fig bigger}}
...
Recently, \citet{2017arXiv170608990H} showed that the early light curve of SN\,2017cbv has a clearly resolved bump in the $U$, $B$, and $g'$ bands in the $\sim$5 d after explosion. \citeauthor{2017arXiv170608990H} find that the bump can be explained via the combination of ejecta-companion interaction and the normal evolution of a type Ia SN. A challenge for this model is that it significantly overpredicts the UV brightness of the SN compared to what is observed. It is argued in \citet{2017arXiv170608990H} that the shock resulting from the companion collision may not be a blackbody, which could reconcile the model with the observed UV photometry. Qualitatively, \abc\ and SN\,2017cbv exhibit similar characteristics: both feature strong \ion{C}{2} absorption in their early spectra and a bump in their early light curves. Below, we argue for circumstellar interaction and strong $^{56}$Ni mixing as a possible explanation for \abc, and indeed \citet{2017arXiv170608990H} consider these possibilities for SN\,2017cbv as well. While there are arguments in favor of and against each of these possibilities, we believe that the early behavior of SN\,2017cbv and \abc\ reflect a common physical origin.
\amiller{should this paragraph be in the conclusions?}
