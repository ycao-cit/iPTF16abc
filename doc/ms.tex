...
Photometric observations in the $BVg'r'i'$ filters were conducted by the Las Cambres Observatory (LCO) 1-m
...
\abc, we fit the P60 light curves with the \texttt{sncosmo} software package.\footnote{The
  ...
  \url{https://sncosmo.readthedocs.io/en/v1.5.x/}.} This fit includes a \texttt{SALT2} template \citep{2007A&A...466...11G} that has been corrected for extinction using the \citep{1999PASP..111...63F} reddening law, $R_V=3.1$, and $E(B-V)$ values from \S\ref{sec:reddening}.
...
\begin{figure*}[!thb]
  ...
  \caption{Constraints on $t_{exp}$ from fitting the velocity
    ...
    velocities (blue circles) and the best-fit power-law model
     ...
     are shown. \textit{Right inset}: Evolution of the 
...
\software{\texttt{PTFIDE} \citep{Masci2017PASP}, \texttt{SALT2}, \texttt{SNID}, \texttt{lcogtsnpipe}, \texttt{UVOTSOURCE}}.
